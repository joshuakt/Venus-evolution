...
@njit # function for finding surface temperature that balances ASR and interior heatflow
def funTs_general(Ts,Tp,ll,visc,beta,Te_input,ASR_input,H2O_Pressure_surface,CO2_Pressure_surface,ocean_CO3,MMW,PO2_surf): 
    if Ts[0]<Tp:
        Ra =2e-5 * 9.8 * (Tp -Ts[0]) * ll**3 / ( 1e-6  * visc)
        qm = (4.2/ll) * (Tp - Ts[0]) * (Ra/1.1e3 )**beta
    else:
        qm = - 2.0 * (Ts[0] - Tp) / ll       
    Ts_in= Ts[0]
    [OLR_new,newpCO2,ocean_pH,ALK,Mass_oceans_crude] = correction(Ts_in,Te_input,H2O_Pressure_surface,CO2_Pressure_surface,6371000.0,9.8,ocean_CO3,1e5,MMW,PO2_surf) 
    heat_atm = OLR_new - ASR_input 
    print (Ts,OLR_new,ASR_input,qm)
...
init_guess = np.array([23.0])
bounds_op =  np.array([0.0, 100.0]).T
#np.array([[0.0], [100.0]]).T
print (bounds_op.shape)
ace_test =  nelder_mead(funTs, x0=init_guess, bounds=np.array([[-500.0], [500.0]]).T,args=(3.0,4.5))#,tol_f=1e-10,tol_x=1e-10, max_iter=1000)
init_guess = np.array([200.0])
betta = 1./3.
ace_test2 =  nelder_mead(funTs_general, x0=init_guess, bounds=np.array([[100.0], [4000.0]]).T,args=(1600.0,2971000.0,1e20,betta,210.0,220.0,1e6,1e4,1e-3,0.044,1e3))#,tol_f=1e-10,tol_x=1e-10, max_iter=1000)
